        ...
        if has_name and has_price:
            ...
            for j, c in enumerate(nc):
                ...
                if (("商品cd" in c) or ("商品コード" in c)) and "商品CD" not in colmap:
                    colmap["商品CD"] = j
                # 処方番号列の見出し表記ゆれ：処方／試作／初法 ＋ 番号／ナンバー／No.(№) の各種を全部認識
                if ((("処方" in c) or ("試作" in c) or ("初法" in c))
                        and (("番号" in c) or ("ナンバー" in c) or ("no" in c))) \
                        and "試作番号" not in colmap:
                    colmap["試作番号"] = j
                if ("売上単価" in c or c == "単価") and "単価" not in colmap:
                    colmap["単価"] = j
                # 価格更新は「価格更新／単価更新」の表記ゆれあり
                if (("価格更新" in c) or ("単価更新" in c)) and "価格更新" not in colmap:
                    colmap["価格更新"] = j
